...
// Initial mock data with maxDevices & boundDevices support
const DEFAULT_KEYS = [
  {
    id: 'key_1',
    ...
    maxDevices: 1,
    boundDevices: ['HWID-DEV-ALEX-991']
  },
  {
    id: 'key_2',
    ...
    maxDevices: 1,
    boundDevices: ['HWID-DEV-BUDI-404']
  },
  {
    id: 'key_3',
...
function writeData(data) {
  ...
  try {
    ...
    // Persistent memory fallback for Vercel serverless function instances
  }
}
...
export function createKey({ username, alias, durationDays, maxDevices }) {
  const keys = readData();
  const days = parseInt(durationDays, 10) || 30;
  const maxDevCount = maxDevices !== undefined ? parseInt(maxDevices, 10) : 1;
  const now = new Date();
...
export function resetKeyHwid(keyIdentifier) {
  const keys = readData();
  const item = keys.find((k) => k.id === keyIdentifier || k.key === keyIdentifier || k.token === keyIdentifier);
...
/**
 * Integrated Pipeline Validation with Device Binding (HWID Lock Support):
 * Web API Key -> User Token -> HWID Check -> SUCCESS
 */
export function validateIntegratedPipeline({ webApiKey, userToken, username, hwid, deviceId }) {
  ...
  // --- Device Lock / HWID Verification Logic ---
  const currentHwid = hwid || deviceId;
  const maxDev = item.maxDevices !== undefined ? item.maxDevices : 1;
  item.boundDevices = item.boundDevices || [];

  if (currentHwid && currentHwid.trim() !== '') {
    const deviceTag = currentHwid.trim();
    const isBound = item.boundDevices.includes(deviceTag);

    if (!isBound) {
      // Check if maxDevices limit reached (0 means unlimited)
      if (maxDev > 0 && item.boundDevices.length >= maxDev) {
        return {
          ...
          maxDevices: maxDev,
          boundDevices: item.boundDevices,
          message: `DEVICE LOCK ERROR: Token ini telah terikat pada batas ${maxDev} Device (HWID: ${item.boundDevices.join(', ')}). Minta Admin Reset Device!`
        };
      } else {
        // Auto-bind device on first valid login
        item.boundDevices.push(deviceTag);
      }
    }
  }
